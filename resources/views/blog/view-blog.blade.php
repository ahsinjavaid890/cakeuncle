...
@section('content')
    <div class="row">
        ...
        <div class="col text-end mb-3">
            <a href="{{ url('write-blog')}}?id={{$blog->id}}" class="btn btn-info btn-sm mb-0">{{__('Edit')}}</a>
            <a href="{{ url('delete/blog')}}/{{$blog->id}}" class="btn btn-warning btn-sm mb-0">{{__('Delete')}}</a>
        </div>
    </div>
    <div class="card">
        <div class="card-body">
                ...
                <div class="col-md-6 ms-auto  mt-3 ">
                    ...
                    @if(!empty($users[$blog->admin_id]->photo))
                        <a href="javascript:" class="ms-3 mt-4 avatar rounded-circle border border-secondary">
                            <img alt="" class="p-1" src="{{ url('public'}}/uploads/{{$users[$blog->admin_id]->photo}}">
                        </a>
                    @else
                    ...
                    @endif
                    <a href="{{ url('profile')}}" class=" ms-3 nav-link text-body font-weight-bold px-0">
